                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select" id="lesson_id" name="lesson_id" required>
                            @foreach ($lessons as $lesson)
                                <option value="{{ $lesson->id }}" {{ $feedback->lesson_id == $lesson->id ? 'selected' : '' }}>{{ $lesson->date }} {{ $lesson->time }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="mb-3">
                        <label for="given_by" class="form-label">Given By</label>
                        <input type="text" class="form-control" id="given_by" name="given_by" value="{{ $feedback->given_by }}" required>
                    </div>
                    <div class="mb-3">
